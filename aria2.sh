...
check_new_ver() {
    aria2_new_ver=$(
        {
            wget -t2 -T3 -qO- "https://api.github.com/repos/Niter8263/aria2/releases/latest" ||
    ...
    if [[ -z ${aria2_new_ver} ]]; then
        echo -e "${Error} Aria2 最新版本获取失败，请手动获取最新版本号[ https://github.com/Niter8263/aria2/releases ]"
...
Download_aria2() {
    ...
    DOWNLOAD_URL="https://github.moeyy.xyz/https://github.com/Niter8263/aria2/releases/download/${aria2_new_ver}/aria2-${aria2_new_ver%_*}-static-linux-${ARCH}.tar.gz"
    {
        wget -t2 -T3 -O- "${DOWNLOAD_URL}" ||
            wget -t2 -T3 -O- "https://mirror.ghproxy.com/${DOWNLOAD_URL}"
    } | tar -zx
...
Download_aria2_conf() {
    PROFILE_URL1="https://github.moeyy.xyz/https://raw.githubusercontent.com/Niter8263/aria2/main/"
...
Service_aria2() {
    if [[ ${release} = "centos" ]]; then
        wget -N -t2 -T3 "https://github.moeyy.xyz/https://raw.githubusercontent.com/Niter8263/aria2/main/service/aria2_centos" -O /etc/init.d/aria2 ||
        ...
        wget -N -t2 -T3 "https://github.moeyy.xyz/https://raw.githubusercontent.com/Niter8263/aria2/main/service/aria2_debian" -O /etc/init.d/aria2 ||
...
crontab_update_start() {
    ...
    echo -e "\n0 7 * * * /bin/bash <(wget -qO- https://github.moeyy.xyz/https://raw.githubusercontent.com/Niter8263/aria2/main/tracker.sh) ${aria2_conf} RPC 2>&1 | tee ${aria2_conf_dir}/tracker.log" >>"/tmp/crontab.bak"
